...
### 与 `docs/development/rl实验记录.md` 的关系

- `docs/development/rl实验记录.md`：Isaac Lab 上 A/B/C 策略 **无避障** 的 reach 对比。
...
输出：`log/runtime/vision_smoke/`（RGB、深度可视化、可选回放帧）。
...
场景：`scene_calib_chess.xml`（无杆 + 可移动棋盘）；输出 `log/runtime/calib/chess_sim/`（角点标注图 + `chess_calib_report.json`）。
...
python mujoco/eval_cbf.py --num-episodes 256 --seed 42 \
  --cbf-label "CBF v2.1" \
  --out-dir log/runtime/eval/cbf_capsule_stepA

# 复用同一批 idx（公平对比）
python mujoco/eval_cbf.py \
  --indices-file log/runtime/eval/cbf_fixed_rod/indices_seed42_n256.json \
  --num-episodes 256 --out-dir log/runtime/eval/cbf_capsule_stepA \
  --cbf-label "CBF v2.1"

# P0：无杆对照（Reach 上限 / obstacle tax）
python mujoco/eval_cbf.py --no-obstacle \
  --indices-file log/runtime/eval/cbf_fixed_rod/indices_seed42_n256.json \
  --num-episodes 256 --out-dir log/runtime/eval/norod_seed42

# P1：多 seed 稳健性（各 seed 独立抽样 idx → out-dir/seed_XX/）
python mujoco/eval_cbf.py --seeds 42,0,1,100 --num-episodes 256 \
  --out-dir log/runtime/eval/cbf_multiseed --cbf-label "CBF v2.1"

# 可选：固定同一批 idx 跨 seed（仅复现校验，非稳健性）
python mujoco/eval_cbf.py --seeds 42,0,1,100 --num-episodes 256 \
  --indices-file log/runtime/eval/cbf_fixed_rod/indices_seed42_n256.json \
  --out-dir log/runtime/eval/cbf_multiseed_sameidx --cbf-label "CBF v2.1"

# 仅从 episodes.csv 重算 summary（升级指标后无需重跑仿真）
python mujoco/eval_cbf.py --summarize-only --out-dir log/runtime/eval/cbf_capsule_stepA \
  --cbf-label "CBF v2.1"
```
...
python mujoco/play.py --enable-cbf --verbose --cbf-log log/runtime/mujoco_cbf_v2.jsonl \
...
- **原始输出**：`log/runtime/eval/cbf_fixed_rod/`（`indices_seed42_n256.json`、`episodes.csv`、`summary.json`）
...
> 定义见 `eval_cbf.py` → `summary.md` 第二节；以下摘自 `log/runtime/eval/cbf_fixed_rod/summary.json`（2026-07-07 跑次）。
...
| 评测 idx | — | **复用** `log/runtime/eval/cbf_fixed_rod/indices_seed42_n256.json`（与 v2 同批 256 目标） |

- **时间**：2026-07-07T15:08:41
- **Git commit**：`d2aabe1`
- **原始输出**：`log/runtime/eval/cbf_capsule_stepA/`
...
- **输出**：`log/runtime/eval/cbf_multiseed/`（`seed_XX/` + `multi_seed_summary.md`）
...
> 完整各 seed 明细见 [`log/runtime/eval/cbf_multiseed/multi_seed_summary.md`](../log/runtime/eval/cbf_multiseed/multi_seed_summary.md)。
...
- **输出**：`log/runtime/eval/sdf_challenge_rod/`
...
- **输出**：`log/runtime/eval/sdf_challenge_wall/`
- **扫描**：`scan_count=512, seed=42`
...
- **输出**：`log/runtime/eval/state_softsettle_quick8/`
- **方法**：
...
MUJOCO_GL=egl python mujoco/eval_sdf_challenge.py \
  --indices-file log/runtime/eval/sdf_challenge_rod/challenge_indices.json \
  --max-challenges 8 \
  ...
  --stop-on-success \
  --use-sim-cam \
  --out-dir log/runtime/eval/state_softsettle_quick8 \
  --log-every 1
```
...
- **场景**：`SO-ARM100/Simulation/SO100/mujoco/scene_plus.xml`
- **输出**：`log/runtime/eval/random_rod_sdf_5rounds/`
- **随机种子**：`seed=13`
- **随机杆底座范围**：
  - `x ∈ [0.10, 0.22] m`
  - `y ∈ [0.02, 0.16] m`
...
MUJOCO_GL=egl python mujoco/eval_random_rod_sdf.py \
  ...
  --seed 13 \
  --use-sim-cam \
  --out-dir log/runtime/eval/random_rod_sdf_5rounds
```
...
- **场景**：`SO-ARM100/Simulation/SO100/mujoco/scene_plus.xml`
- **输出**：`log/runtime/eval/dynamic_target_obstacle_large_v1/`
- **随机种子**：`seed=42`
- **target 动态轨迹**：
  - `target_motion=line`
...
MUJOCO_GL=egl python mujoco/eval_sdf_challenge.py \
  ...
  --out-dir log/runtime/eval/dynamic_target_obstacle_large_v1 \
  --log-every 32
```
