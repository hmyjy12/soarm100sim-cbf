...
python hardware/tools/read_feetech7.py \
  ...
  --samples 20 \
  --period 0.5 \
  --log logs/hardware/feetech7_read.jsonl
```
...
python hardware/tools/snapshot_feetech7.py \
  --port /dev/ttyACM0 \
  --output logs/hardware/feetech7_register_snapshot.json
```
...
python hardware/tools/test_single_joint.py \
  ...
  --log logs/hardware/test_wrist_roll_plus1.jsonl \
...
python hardware/tools/stream_policy_joint_udp.py \
  ...
  --udp-port 15001 \
  --rate 20 \
  --log logs/hardware/policy_joint_stream.jsonl
```
...
- 每次动作保存独立 JSONL 到 `logs/hardware/`。
...
saved immediately under `logs/hardware/wrist_handeye/<timestamp>/`.
