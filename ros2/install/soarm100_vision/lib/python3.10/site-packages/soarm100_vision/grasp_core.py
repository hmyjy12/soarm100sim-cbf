...
class GraspThresholds:
    ...
    # The position actuator needs about 1.5 s to settle from open_q=1.2 to
    # close_q=0.4. Do not start lifting while the fingers are still closing.
    close_time: float = 1.60
...
class GraspStateMachine:
    ...
    def step(self, obs: GraspObservation) -> ControlIntent:
        ...
        if self.phase == GraspPhase.REPLAN_GRASP:
            ...
            if self.replan_attempts >= th.replan_max_attempts:
                # IK/replan pose errors do not necessarily describe whether the
                # object is already between the fingers. Try one physical close
                # and let the subsequent lift decide grasp success.
                self.phase = GraspPhase.CLOSE
                self.close_counter = 0
                self.last_reason = (
                    f"replan_attempts_exhausted_close_fallback "
                    f"attempts={self.replan_attempts}"
                )
                return self._intent(
                    plan.grasp_pos,
                    plan.grasp_quat_wxyz,
