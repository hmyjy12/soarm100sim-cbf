...
/home/sophie/miniconda3/bin/python mujoco/play.py \
  ...
  --traj-log logs/external_grasp_smoke.jsonl \
  --verbose \
...
- backend 使用 `logs/calib/camera_calib.json` 中 `scene_depth` 的 `T_parent_cam` 完成转换。
...
pose_to_base_from_calib
  使用 logs/calib/camera_calib.json 做 camera optical -> base/world 转换
...
```text
默认：logs/ros2_inprocess_grasp.jsonl
...
./ros2/run_vision_grasp.sh \
  ...
  --mujoco-target-object cube \
  --mujoco-target-pos 0.42,0.08,0.021 \
  --mujoco-traj-log logs/ros2_inprocess_grasp.jsonl \
  --visualizer off \
  --show-window off
```
...
非 GUI ROS2 in-process action smoke：
  ...
  写出 logs/ros2_inprocess_action_smoke.jsonl，共 450 行
...
```text
logs/ros2_inprocess_gui_init_fixed.jsonl
rows=231
...
```text
旧 logs/calib/camera_calib.json 中 scene_depth:
...
本次结果文件为
`logs/ros2_vision/fixed_yolo_sam/segment_target_latest.json`。测试退出后确认
