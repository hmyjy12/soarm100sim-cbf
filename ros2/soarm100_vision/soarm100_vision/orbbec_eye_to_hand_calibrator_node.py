"""Eye-to-hand calibrator for a fixed Orbbec RGB camera.
...
class OrbbecEyeToHandCalibrator(Node):
    def __init__(self) -> None:
        super().__init__("orbbec_eye_to_hand_calibrator")
        ...
        self._last_status = "ready (eye-to-hand)"
        self._window = "SO-ARM100 Orbbec eye-to-hand (SPACE capture, S solve, Q quit)"
        ...
        self.create_service(Trigger, "/orbbec_handeye/capture", self._handle_capture)
        self.create_timer(1.0 / 15.0, self._tick)
        if self._show_window:
            cv2.namedWindow(self._window, cv2.WINDOW_NORMAL)

        self.get_logger().info(
            "orbbec eye-to-hand ready; "
            ...
            f"reject_duplicates={bool(self.get_parameter('reject_duplicate_samples').value)} "
            f"capture_srv=/orbbec_handeye/capture output={self._session_dir}"
        )
    ...
    def _capture(self) -> tuple[bool, str]:
        ...
        _, T_camera_board, rms = self._detection
    ...
    def _solve(self) -> None:
        ...
        # Eye-to-hand: feed inverse of T_base_gripper into calibrateHandEye.
        R_g2b_inv = []
        t_g2b_inv = []
        R_t2c = []
        t_t2c = []
        for sample in self._samples:
            T_inv = _invert_rigid(sample.T_base_gripper)
            R_g2b_inv.append(T_inv[:3, :3])
            t_g2b_inv.append(T_inv[:3, 3].reshape(3, 1))
            R_t2c.append(sample.T_camera_board[:3, :3])
        ...
        T_base_camera[:3, 3] = np.asarray(t_c2b).reshape(3)

        # Board pose in gripper should be constant for a rigidly held board.
        board_in_gripper = [
            _invert_rigid(s.T_base_gripper) @ T_base_camera @ s.T_camera_board
            for s in self._samples
        ]
        translations = np.stack([T[:3, 3] for T in board_in_gripper])
        center = np.mean(translations, axis=0)
        trans_errors = np.linalg.norm(translations - center, axis=1)
        R_ref = board_in_gripper[0][:3, :3]
        rot_errors = np.array(
            [_rotation_angle_deg(R_ref.T @ T[:3, :3]) for T in board_in_gripper]
        )
        result = {
            ...
            "T_base_camera": _matrix_dict(T_base_camera),
            "validation": {
                "fixed_board_in_gripper_translation_rms_mm": float(
                    np.sqrt(np.mean(trans_errors**2)) * 1000.0
                ),
                "fixed_board_in_gripper_translation_max_mm": float(np.max(trans_errors) * 1000.0),
                "fixed_board_in_gripper_rotation_rms_deg_vs_first": float(
                    np.sqrt(np.mean(rot_errors**2))
                ),
                "fixed_board_in_gripper_rotation_max_deg_vs_first": float(np.max(rot_errors)),
        ...
        v = result["validation"]
        yaml_output = self._session_dir / "orbbec_eye_to_hand_tsai.yaml"
        ...
        storage.write("T_base_camera", T_base_camera)
        storage.write("sample_count", len(self._samples))
        storage.write(
            "fixed_board_in_gripper_translation_rms_mm",
            v["fixed_board_in_gripper_translation_rms_mm"],
        )
        storage.write(
            "fixed_board_in_gripper_rotation_rms_deg",
            v["fixed_board_in_gripper_rotation_rms_deg_vs_first"],
        )
        storage.release()
        message = (
...
def main() -> None:
    rclpy.init()
    node = OrbbecEyeToHandCalibrator()
    try:
        rclpy.spin(node)
